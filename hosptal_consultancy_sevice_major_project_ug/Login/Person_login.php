    ...
    <div class="dropdown-content">
      ...
      <a href="Person_login.php">PERSON LOGIN</a>
       ...
       <div class="div1">
           ...
           <br><!-- comment --><br>
           <input type="text" name="id" class="form-control" placeholder="ENTER YOUR ID" autocomplete="off" required><br><br>
           <input type="password" name="pass" class="form-control" placeholder="ENTER YOUR PASSWORD" autocomplete="off" required><br><br>
           
               <button type="submit"  class="btn btn-info" style="     width: 62%;
    height: 55px;
    margin-left: 117px;
    float: left;
    margin-top: 27px;" name="check" >SUBMIT</button><br>
    <br>
    <a href=" ../Manager/User_creation/Create_user_person.php" align="center" style="    margin-left: 256px;">create an account!!!</a>
                  </div>
       </div>
        ...
        if(isset($_POST['check']))
        {
            $id=$_POST['id'];
             $pass=$_POST['pass'];
              $sql    = "SELECT * FROM  common_login WHERE pid='$id' ";
        $res    =   mysqli_query($con, $sql) or die(mysqli_query($con));                                               
        $row1 =mysqli_fetch_array($res);
        
            
             if(md5($pass)==$row1['ppassword']){
                 
                 $_SESSION['pid']=$row1['pid'];
             echo "<script>window.location='../person/index.php'</script>";
