          ...
          $date=  date("Y-m-d");
            $time= date("H:i:s");
              ...
              $r= base64_encode($recid);   
                 $rid=$_GET['rid'];
    $sql    = " insert into e_letter_box (from_1,to_1,recr_id,type,message,date,time,rdate,rtime,raddress) VALUES ('$creator_id','$to','$recid','$title','$content','$date','$time','$rdate','$rtime','$raddress')";
       $res    =   mysqli_query($con, $sql) or die(mysqli_query($con));                                               
